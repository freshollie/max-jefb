...
combs = sorted(combs, key=lambda comb: len(set(comb[0])) if "rouge" in sys.argv else comb[1] - (len(set(comb[0])) / 10), reverse=True)
...
for comb in combs[0:50 if "rouge" not in sys.argv else 50]:
